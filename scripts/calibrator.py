...
def train(cfg) :
    ...
    checkpoints = LearnerWorker.get_checkpoints(LearnerWorker.checkpoint_dir(cfg, policy_index))
...
def test(cfg):
    ...
    calibrator = create_calibrator(cfg, env.observation_space)
    calibrator_dir = f"train_dir/calibrator_dir/c{policy_index}"
    calibrator_path = os.path.join(calibrator_dir, "checkpoint_p0.pth")
